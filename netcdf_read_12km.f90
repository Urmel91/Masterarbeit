...
!------ Beschreibung ------------------------------------------------
! In diesem File werden die -nc-Dateien zu .txt umgewandelt. Wichtig 
! ist hier, dass die erste Datei nur 5 Jahre, also 60 Monate abdeckt,
! das Array tas_in also eine andere Größe hat. Deswegen sind tas_in1
! und tas_in2 erstellt worden und es finden sich einige if-Bedingungen
! in dem Code, die hier nur für die unterschiedliche Länge der Arrays 
! eingebaut wurden. Soll nur eine Datei eingelesen werden, können diese
! IF-Bedinungen entfernt werden. 

!----- Nutzung ------------------------------------------------------
! LAT_START, LAT_END legen Anfang und Ende der Breitengrade fest, die 
! benötigt werden. Analog LON. misslats sind Breitengrade, die in der
! gewählten Umgebung liegen, aber nicht mehr zu Niedersachsen gehören.

!----- Datei-Info----------------------------------------------------
! Die Datei mit _0 am Ende geht von 1971-1980/12, mit _1 von 1981-1990/12, 
! mit _2 von 1991-2000/12, mit _3 von 2001-2005/12 (also bis anfang 2006)
...
program netcdf_read
  ...
  character (len = (len(FILE_IN)+4)) :: FILE_NAME
  character (len = (len(FILE_IN)+5)) :: FILE_OUT
...
DO p = 0, 3  
    ...
    write (FILE_NAME,"(A,I1,A3)") FILE_IN, p, ".nc"     
...
!----- Einlesen in .txt-File -------------------------
    write (FILE_OUT, "(A,I1,A4)") FILE_IN, p, ".txt"

    open(21+p,file=FILE_OUT,status='replace',action='write')
        ...
        do g = 1, numTimes
            do i = lat_start, lat_end
                do j = lon_start, lon_end
                    if (lat_lon(i,j)%n == 1) then
                        if (mod(g,2) == 0) then
                            write(21+p,'(I0,F8.5,1X,F0.5,1X,F0.5)') g, lat_lon(i,j)%lat_rot, lat_lon(i,j)%lon_rot,&
